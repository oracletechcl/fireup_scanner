...
def test_review_point(capsys):     
    ...
    result_dictionary = LBaaSHealthChecks(Statics.__rp_2_13['entry'], 
    ...
    results_in_fault=0
    dictionary = result_dictionary.analyze_entity(Statics.__rp_2_13['entry'])   

    for item in dictionary[Statics.__rp_2_13['entry']]['findings']:
        debug_with_date(item)
        results_in_fault += 1

    assert results_in_fault == 14
