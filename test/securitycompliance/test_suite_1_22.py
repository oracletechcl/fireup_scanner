...
def test_review_point(capsys):     
    ...
    result_dictionary = NetworkSources(Statics.__rp_1_22['entry'], 
    ...
    results_in_fault=0
    dictionary = result_dictionary.analyze_entity(Statics.__rp_1_22['entry'])   
    
    for item in dictionary[Statics.__rp_1_22['entry']]['findings']:
        debug_with_date(item)
        results_in_fault += 1    

    assert results_in_fault == 1
