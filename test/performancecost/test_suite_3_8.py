...
def test_review_point(capsys):
    result_dictionary = ImplementCostTrackingTags(Statics.__rp_3_8['entry'],
    ...
    results_in_fault = 0
    dictionary = result_dictionary.analyze_entity(Statics.__rp_3_8['entry'])

    for item in dictionary[Statics.__rp_3_8['entry']]['findings']:
        debug(item)
        results_in_fault += 1
