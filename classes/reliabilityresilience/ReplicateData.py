...
class ReplicateData(ReviewPoint):
    ...
    __block_volumes = []
    __boot_volumes = []
    __storages_with_no_replication = []
    __file_systems = []
    ...
    def load_entity(self):
        ...
        if len(self.__block_volumes + self.__boot_volumes) > 0:
            self.__storages_with_no_replication = parallel_executor(block_storage_clients, self.__block_volumes + self.__boot_volumes, self.__search_for_policy, len(self.__block_volumes + self.__boot_volumes), "__storages_with_no_policy")

        return self.__storages_with_no_replication
